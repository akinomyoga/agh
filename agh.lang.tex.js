...
_Mod.CHRelax=function(doc,cmdName){doc.scanner.Next();};
/*
...
new function(){
      ...
      case "Fx-std":
        return function(imgsrc,width,alt,className){
          if(className in stretchSvgPath){
            ...
            return '<td rowspan="2" class="aghtex-stretch-svg aghtex-stretch-svg-'+className+'">'+svg+'</td>';
          }else{
            // 旧来の .png を用いた表示
            width=width.toString()+"ex";
            //return '<td rowspan="2" style="width:'+width+';background-image:url('+ns.BaseUrl+imgsrc+');-moz-background-size:100%;"></td>';
            return '<td rowspan="2" class="aghtex-stretch" style="height:100%;"><img src="'+ns.BaseUrl+imgsrc+'" alt="'+alt+'" style="width:'+width+';height:100%;"/></td>';
          }
        };
      case "Sf-qks":
      case "Sf-std":
        return function(imgsrc,width,alt,className){
          if(className in stretchSvgPath){
            ...
            if(className)
              return '<td rowspan="2" class="aghtex-stretch-bg aghtex-stretch-bg-'+className+'"></td>';
            else
              return '<td rowspan="2" class="aghtex-stretch-bg" style="width:'+width+';background-image:url('+ns.BaseUrl+imgsrc+');"></td>';
          }
        };
      case "Op-qks":
      case "Op-std":
        return function(imgsrc,width,alt){
          width=(width*0.8).toString()+"ex";
          return '<td rowspan="2" class="aghtex-stretch" style="width:'+width+';background-image:url('+ns.BaseUrl+imgsrc+');"></td>';
        };
      default:
        return function(imgsrc,width,alt){
          width=(width*0.8).toString()+"ex";
          return '<td rowspan="2" class="aghtex-stretch" style="width:'+width+';background-image:url('+ns.BaseUrl+imgsrc+');background-size:100% 100%;"></td>';
        };
    }
  })();
  //------------------------------------------------------------
  ...
  _Mod.OutputStretchBracketTd=(function(){
    var bracket_dict={
      ...
      //"<tex:f>&#xF0E9;</tex:f>":'<td rowspan="2" class="tex-left-ceil">&nbsp;</td>',
      //"<tex:f>&#xE9;</tex:f>":'<td rowspan="2" class="tex-left-ceil">&nbsp;</td>',
      //"<tex:f>&#xF0EB;</tex:f>":'<td rowspan="2" class="tex-left-flor">&nbsp;</td>',
      //"<tex:f>&#xEB;</tex:f>":'<td rowspan="2" class="tex-left-flor">&nbsp;</td>',
      //"<tex:f>&#xF0F9;</tex:f>":'<td rowspan="2" class="tex-right-ceil">&nbsp;</td>',
      //"<tex:f>&#xF9;</tex:f>":'<td rowspan="2" class="tex-right-ceil">&nbsp;</td>',
      //"<tex:f>&#xF0FB;</tex:f>":'<td rowspan="2" class="tex-right-flor">&nbsp;</td>',
      //"<tex:f>&#xFB;</tex:f>":'<td rowspan="2" class="tex-right-flor">&nbsp;</td>',
...
new function(){
  ...
  _Ctx.DefineCommand({"newlength":['f;#@1',function(doc,argv){
    var lname=get_lenName(doc,argv[1]);
    if(lname==null)return;
    doc.NewLength(lname);
  }]});
  _Ctx.DefineCommand({"setlength":['f;#@1#!2',function(doc,argv){
    var lname=get_lenName(doc,argv[1]);
    if(lname==null)return;
    setlength(doc,lname,argv[2]);
  }]});
  _Ctx.DefineCommand({"addtolength":['f;#@1#!2',function(doc,argv){
    var lname=get_lenName(doc,argv[1]);
    if(lname==null)return;
 ...
 *    @fn mod_common.CreateCommandTagFollowing(doc,tagName[,endTag])
 */
...
new function(){
  ...
  _Ctx.DefineCommand({"hspace*":['s@;#1','<span style="padding-right:#1;"></span>']});
  _Ctx.DefineCommand({"vspace":['s@;#1','<p class="tex-vspace" style="font-size:#1">&nbsp;</p>']});
  _Ctx.DefineCommand({"vspace*":['s@;#1','<p class="tex-vspace" style="font-size:#1">&nbsp;</p>']});
  ...
  //---------------------------------------------------------------------------
  // \mbox
  _Ctx.DefineCommand({"mbox":['s@;#mode.lr>1','<tex:mbox>#1</tex:mbox>']});
  _Ctx.DefineCommand({"fbox":['s@;#mode.lr>1','<tex:mbox class="aghtex-latex-fbox">#1</tex:mbox>']});
  _Ctx.DefineCommand({"frame":['s@;#mode.lr>1','<tex:mbox class="aghtex-latex-frame">#1</tex:mbox>']});
  ...
  _Ctx.DefineCommand({"string":['f',function(doc,argv){
    doc.skipSpaceAndComment();
...
new function(){
  ...
  _Ctx.DefineCommand({
    bf : mod_common.CreateCommandTagFollowing('<tex:i class="aghtex-mathbf">' ,'<tex:i>'),
    it : mod_common.CreateCommandTagFollowing('<tex:i class="aghtex-mathit">' ,'<tex:i>'),
    rm : mod_common.CreateCommandTagFollowing('<tex:i class="aghtex-mathrm">' ,'<tex:i>'),
    sf : mod_common.CreateCommandTagFollowing('<tex:i class="aghtex-mathsf">' ,'<tex:i>'),
    tt : mod_common.CreateCommandTagFollowing('<tex:i class="aghtex-mathtt">' ,'<tex:i>'),
    cal: mod_common.CreateCommandTagFollowing('<tex:i class="aghtex-mathcal">','<tex:i>'),

    // - 以下は数式モードでは効果無し。
    mc          : mod_base.CHRelax,
    gt          : mod_base.CHRelax,
    normalfont  : mod_base.CHRelax,
    tiny        : mod_base.CHRelax,
    scriptsize  : mod_base.CHRelax,
    footnotesize: mod_base.CHRelax,
    small       : mod_base.CHRelax,
    normalsize  : mod_base.CHRelax,
    large       : mod_base.CHRelax,
    Large       : mod_base.CHRelax,
    LARGE       : mod_base.CHRelax,
    huge        : mod_base.CHRelax,
    Huge        : mod_base.CHRelax,
  ...
  _Ctx.DefineLetter({".":['f@',function(doc,cmdName){
    var html=cmdName;
    doc.scanner.Next();
    if(doc.scanner.wordtype=="ltr"&&" \b\t\v\r\n\f".indexOf(doc.scanner.word)>=0){
  ...
  if(agh.browser.vIE&&agh.browser.vIE<7){
    _Ctx.DefineCommand({"!":['s@','<tex:negspace class="letter-spacing">&nbsp;&nbsp;</tex:negspace>&nbsp;']});
  }else{
    _Ctx.DefineCommand({"!":['s@','<tex:negspace></tex:negspace>']});
  }
  ...
  if(agh.browser.vIE<=6||ns.compatMode=="IE-qks"){
    ...
    function cmd_supsub_output(doc,sup,sub){
    ...
    _Ctx.DefineLetter({"^":['f;#>1',function(doc,argv){
      doc.skipSpaceAndComment();
      if(doc.scanner.is("ltr","_")){
        doc.scanner.Next();
        cmd_supsub_output(doc,argv[1],doc.GetArgumentHtml());
      }else{
        doc.currentCtx.output.buff.push('<tex:sup class="aghtex-tag-script"><tex:i class="aghtex-sup-nest">',argv[1],'</tex:i></tex:sup>');
      }
    }]});
    _Ctx.DefineLetter({"_":['f;#>1',function(doc,argv){
      ...
      if(doc.scanner.is("ltr","^")){
        doc.scanner.Next();
        cmd_supsub_output(doc,doc.GetArgumentHtml(),argv[1]);
      }else{
        doc.currentCtx.output.buff.push('<tex:sub class="aghtex-tag-script">',argv[1],'</tex:sub>');
      }
    }]});
  }
  ...
  if(agh.browser.vIE<=8||ns.compatMode=="IE-qks"){
    _Ctx.DefineCommand({"frac":['s@;#>1#>2','<table cellspacing="0" class="aghtex-frac"><tr><td class="tex-num">#1</td></tr><tr><td class="tex-den">#2</td></tr></table>']});
    _Ctx.DefineCommand({"over":['f',function(doc,argv){
      var output=doc.currentCtx.output;
      var former=output.toHtml();
      output.clear();
      output.buff.push('<table cellspacing="0" class="aghtex-frac"><tr><td class="tex-num">',former,'</td></tr><tr><td class="tex-den">');
  ...
  function Atop(output,left,right,bar){
    ...
    buff.push('<td class="aghtex-center"');
    if(bar)
      buff.push(' style="border-bottom:1px solid black;"');
    buff.push('>',former,'</td>');

    if(right)
      mod_base.OutputStretchBracketTd(output,right,2);
    buff.push('</tr><tr><td class="aghtex-center">');
    output.appendPost('</td></tr></table>');
  }
  _Ctx.DefineCommand({"atopwithdelims":['f;#!1#!2',function(doc,argv){
    Atop(
      doc.currentCtx.output,
      argv[1],argv[2]
      );
  }]});
  ...
  _Ctx.DefineCommand({"mathop":['f;#>1',function(doc,argv){
    ...
    if(!_Mod.GetMathStyle(doc)){
      ...
      if(sbsp.sub==null&&sbsp.sup==null){
        ...
        buff.push(
          '<tex:mathop><tex:u class="aghtex-tag-script">',sbsp.sup||'&nbsp;',
          '</tex:u>',argv[1],
          '<tex:d class="aghtex-tag-script">',sbsp.sub||'&nbsp;',
          '</tex:d></tex:mathop>');
      }
    }else{
      buff.push(argv[1]);
...
new function(){
  ...
  (function(){
    var STACKREL_DEFAULT_TOP=agh.browser.select({IE:0,Fx:0,Op:0,Cr:0.2,Sf:0.8,Def:0.2}); // 本当の所 0.1 にしたい
    function is_single_lower(html){
      var text=agh.Text.Unescape(html,"html");
      if(text.length!=1)return false;
      return "acegijmnopqrsuvwxyzαγεηικμνοπρστυφχψω\u0131".indexOf(text)>=0;
    }
    ...
    _Ctx.DefineCommand({"stackrel":['f;#>1#>2',function(doc,argv){
      var t=STACKREL_DEFAULT_TOP-1.3;
      if(is_single_lower(argv[1]))t+=0.45;
      t=0.01*Math.round(t*100);

      var buff=doc.currentCtx.output.buff;
      buff.push(
        '<tex:lap><tex:m style="top:',t,'ex;"><tex:small3>',argv[1],
        '</tex:small3></tex:m><tex:m>',argv[2],'</tex:m></tex:lap>');
    }]});
    ...
    if(ns.compatMode=="IE-qks"||agh.browser.vIE<8){
      _Mod.CreateAccentCommandQksB=function(height,htSymbol){
        ...
        // 00:28 2011/02/23 補正
        if(agh.browser.vCr)
          TAG_L='<tex:lap style="vertical-align:0.684ex;"><tex:m style="top:';

        var style_top=STACKREL_DEFAULT_TOP-height;
        return new ns.Command2("f","#>1",function(doc,argv){
          var t=style_top;
...
new function(){
  ...
  if(agh.browser.vIE&&agh.browser.vIE<7){
    _Ctx.DefineCommand({"negthinspace":['s@','<tex:negspace class="letter-spacing">&nbsp;&nbsp;</tex:negspace>&nbsp;']});
  }else{
    _Ctx.DefineCommand({"negthinspace":['s@','<tex:negspace></tex:negspace>']});
  }
  ...
  _Ctx.DefineCommand({" ":['s@',"&nbsp;"]});
  _Ctx.DefineCommand({"\\":['s@',"<br />"]});
  _Ctx.DefineCommand({"par":['s@',"<br />"]});
  _Ctx.DefineLetter({'\n':['f',function(doc,argv){
  ...
  _Ctx.AddEnvironment("center",ns.Environment.Create("s@",null,'<div class="tex-center">#0</div>',_CtxName));
  _Ctx.AddEnvironment("flushright",ns.Environment.Create("s@",null,'<div class="tex-right">#0</div>',_CtxName));
  _Ctx.AddEnvironment("flushleft",ns.Environment.Create("s@",null,'<div class="tex-left">#0</div>',_CtxName));
  _Ctx.AddEnvironment("quote",ns.Environment.Create("s@",null,'<blockquote class="tex-quote">#0</blockquote>',_CtxName));
  _Ctx.AddEnvironment("quotation",ns.Environment.Create("s@",null,'<blockquote class="tex-quota">#0</blockquote>',_CtxName));
...
ns.Document.prototype.AddContentsLine=function(doc,file,type,content,id){
  ...
  case "toc":
    ...
    f.buff.push(
      '<tex:i class="aghtex-toc-item"><table class="aghtex-toc-item aghtex-toc-',type,
      '"><tr><td class="aghtex-toc-label">',content,
      '</td><td class="aghtex-toc-hfill"><tex:i class="aghtex-toc-hfill">&nbsp;</tex:i></td><td class="aghtex-toc-page">');
  ...
  ResolveReferences:function(){
    ...
    return this.html=this.html.replace(/<tex:ref ref="(\w+)\.([^"<>]+)">\?<\/tex:ref>|<a class="aghtex-ref" href="#aghtex\.([^"]+)">/g,function($0,$1,$2,$B1){ //
      if($1!=null&&$1!=""){
        switch($1){
        case "ref":
          return self.references.displayedText['ref@'+$2]||$0;
        case "bib":
          return self.references.displayedText['bib@'+$2]||$0;
        case "contents":
          if($2=="toc"||$2=="lof"||$2=="lot"){
            var f=self["mod:ref/"+$2];
...
agh.memcpy(_at.Cell.prototype,{
  ...
  get_bL:function(){
    return this.borderL||this.y==0&&this.table.col(0).borderL;//borderV[0];
  },
  get_bR:function(){
    return this.borderR||this.table.col(this.y).borderR;//borderV[this.y+1];
  },
  get_bT:function(){
  ...
  /*
    if(this.y==8){
      console.log("Now going to write cell.");
      console.log("Cell content is"+this.content+".");
      console.log(this.get_bL());
    }
  //*/
    bL=bL||[];
    bR=bR||[];
    ...
    var pL=this.get_pL();
    if(pL!=null){
      sbuff.push('padding-left:',pL,';');
    }
    var pR=this.get_pR();
    if(pR!=null){
      sbuff.push('padding-right:',pR,';');
    }
  ...
  //*/
    ...
    // 本体書き出し
    var buff=output.buff;
    buff.push(' <td class="aghtex-array-cell"');
    buff.push(_at.toAlign(this.get_align()));
    if(tdspan>1){
      buff.push(' colspan="',tdspan,'"');
    }
  ...
  write_bL:function(output,bL,bT,bB){
    ...
    for(var i=0,iN=bL.length;i<iN;i++){
      ...
      if(typeof border=="string"||border instanceof String){
        buff.push(
          ' <td class="aghtex-array-border-txt" style="border-width:',
        ...
        if(bw_left>0){
          buff.push(
            ' <td class="aghtex-array-border-zw" style="border-width:',
            bw_top,'px ',bw_right,'px ',bw_bottom,'px ',bw_left,'px;"></td>\n');
        }
...
new function(){
  ...
  _Ctx.DefineLetter({"lcr":['f@',function(doc,cmdName){
    doc.scanner.Next();
  ...
  _Ctx.DefineLetter({"p":['f;#..!1',function(doc,argv){
    var setter=_at.AlignHandlers.getAlignSetter(doc,'p');
    if(setter!=null)setter.setWidth(argv[1]);
  }]});
  _Ctx.DefineLetter({"@":['f;#..>1',function(doc,argv){
    var setter=_at.AlignHandlers.getAlignSetter(doc,'@');
    if(setter!=null)setter.setBorder(argv[1]);
  }]});
  _Ctx.DefineLetter({"*":['f;#..!1#@2',function(doc,argv){
    var output=doc.currentCtx.output;
  ...
  _Ctx.DefineLetter({"!\"#$'()=~-^`{[;:]+,./&<>?_&<>":['f@',function(doc,cmdName){
    doc.scanner.Next();
    _at.AlignHandlers[0](doc,cmdName);
  }]});
  _Ctx.DefineLetter({"\0\b\t\n\v\f\r 　":['f@',function(doc,cmdName){doc.scanner.Next();}]});
  _Ctx.DefineLetter({"\x01\x02\x03\x04\x05\x06\x07\x0e\x0f\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f":['f@',function(doc,cmdName){
    doc.scanner.Next();
  }]});
}
/*
...
agh.memcpy(_at.Table.prototype,{
  ...
  write:function(output){
    ...
    if(this.param_bracket)
      buff.push('<table class="aghtex-array aghtex-array-valign-',this.valign,' aghtex-array-lrpad">\r\n');
    else
      buff.push('<table class="aghtex-array aghtex-array-valign-',this.valign,'">\r\n');
    ...
    for(var x=0;x<this.xM;x++){
      if(x==0)
        buff.push('<tr class="aghtex-array-first-row">\n');
      else if(x==this.xM-1)
        buff.push('<tr class="aghtex-array-last-row">\n');
      else
        buff.push('<tr>\n');
  ...
  write_thead:function(output){
    ...
    if(agh.browser.vFx){
      buff.push('<tr class="aghtex-array-fxdummy">\n');
      for(var y=0;y<this.yM;y++){
        var col=this.col(y);
        buff.push(' ');
        for(var i=0;i<col.tdcL;i++)buff.push('<td />');
        buff.push('<td style="width:auto;"><div>.</div></td>');
        for(var i=0;i<col.tdcR;i++)buff.push('<td />');
        buff.push('\n');
      }
      buff.push('</tr>\n');
    }
  },
  write_bT:agh.browser.vIE?function(output,tdc_total){
    var h=this.getHorizontalLines(0)-1;
    while(h-->0){
      output.buff.push(
        '<tr><td class="tex-arr2hlineT" colspan="',tdc_total,
        '"><tex:i class="aghtex-array-hline-iedummy">&nbsp;</tex:i></td></tr>\n');
    }
  }:function(output,tdc_total){
    var h=this.getHorizontalLines(0)-1;
    while(h-->0){
      output.buff.push(
        '<tr><td class="tex-arr2hlineT" colspan="',tdc_total,
  ...
  write_bB:agh.browser.vIE?function(output,tdc_total,x){
    var h=this.getHorizontalLines(x+1)-1;
    while(h-->0){
      output.buff.push(
        '<tr><td class="tex-arr2hlineB" colspan="',tdc_total,
        '"><tex:i class="aghtex-array-hline-iedummy">&nbsp;</tex:i></td></tr>\n');
    }
  }:function(output,tdc_total,x){
    var h=this.getHorizontalLines(x+1)-1;
    while(h-->0){
      output.buff.push(
        '<tr><td class="tex-arr2hlineB" colspan="',tdc_total,
...
agh.memcpy(_at.Table,{
  ...
  H_NEXT_ROW:function(doc,cmdName){
    doc.scanner.Next();
    var output=doc.currentCtx.output;
    var data=_at.Table.GetTable(doc,cmdName,"NextLine");
    if(data==null)return;
...
new function(){
  ...
  _Ctx.DefineCommand({"\\":['f@',function(doc,cmdName){
    doc.scanner.Next();
    var output=doc.currentCtx.output;
    var data=_at.Table.GetTable(doc,cmdName,"NextLine");
    if(data==null)return;
...
ns.Document.Classes["revtex4"]=function(doc){
  initialize_article_counters(doc);
...
new function(){
  ...
  _Ctx.DefineCommand({
    mathbb             : ['s@;#>1',(
      // Sf/Cr: text-stroke
      agh.browser.vSf?'<tex:i class="aghtex-mathbb-stroke">#1</tex:i>':
      // Cr/Op/Fx: text-shadow
      agh.browser.vCr||agh.browser.vOp||agh.browser.vFx>=3.5?'<tex:i class="aghtex-mathbb-shadow">#1</tex:i>':
      // IE: filter:glow
...
new function(){
  ...
  _Ctx.DefineCommand({
    ...
    // 色々なスタイルの分数
    dfrac     : ['f;#>1#>2',mod_math.CH_FRAC],
    tfrac     : ['f;#>1#>2',mod_math.CH_FRAC]
  });
  ...
  _Ctx.DefineCommand({
    ...
    DOTSB: mod_base.CHRelax,
    DOTSI: mod_base.CHRelax,
    DOTSX: mod_base.CHRelax
  });
...
new function(){
  ...
  _Ctx.DefineCommand({"color":['f;#[]!1#!2',function(doc,argv){
    ...
    doc.currentCtx.output.buff.push('<tex:i class="aghtex-color-textcolor" style="color:',color,'">');
  ...
  _Ctx.DefineCommand({"textcolor":['f;#[]!1#!2#>3',function(doc,argv){
    ...
    doc.currentCtx.output.buff.push('<tex:i class="aghtex-color-textcolor" style="color:',color,'">',argv[3],"</tex:i>");
  }]});
  _Ctx.DefineCommand({"colorbox":['f;#[]!1#!2#mode.lr>3',function(doc,argv){
    ...
    if(bgc!=null){
      buff.push('<tex:mbox class="aghtex-color-box" style="background-color:',bgc,'">');
    }else{
      buff.push('<tex:mbox class="aghtex-color-box">');
    }
    buff.push(argv[3],"</tex:mbox>");
  }]});
  _Ctx.DefineCommand({"fcolorbox":['f;#[]!1#!2#!3#mode.lr>4',function(doc,argv){
    ...
    if(frc!=null||bgc!=null){
      buff.push('<tex:mbox class="aghtex-color-box" style="');
      if(frc!=null){
