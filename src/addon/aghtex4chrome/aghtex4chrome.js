...
(function() {
  ...
  function getDocumentLastModifiedDate(_document) {
  ...
  function init_site_texfile(eSrc) {
    ...
    var texfile = _window.location.href.replace(/#.*$/, "");
    var islocal = /^file:\/\//.test(texfile);

    ext.connection = chrome.extension.connect();

    var source = {
      update_page_position: function() {
        if (/^#/.test(_window.location.hash)) {
          if (!this.anchor) this.anchor = _document.createElement("a");
          this.anchor.href = _window.location.hash;
          this.anchor.click();
        }
      },
      update_async: function(content, content_mtime, type) {
        if (this.flag_process) return false;
        ...
        if (this.contentElement == null) {
          ...
          this.lastModified = new Date().toUTCString(); // last conversion date

          var option = {lastModified: content_mtime.toISOString()};
          // async tex_transform(this.content, "full_texview", option);
          BackgroundCall(ext.connection, "tex_transform", [this.content, "full_texview", option], function(info) {
            e_content.innerHTML = info.html;
            eSrc.parentNode.insertBefore(e_content, eSrc);
            self.set_toc(info.toc);
            if (type !== "reload") self.update_page_position();
    ...
    source.update_async(eSrc.textContent, getDocumentLastModifiedDate(eSrc.ownerDocument));
        ...
        if (http.overrideMimeType) {
          ...
          if (buff != null) {
            ...
            BackgroundCall(ext.connection, "agh.Text.Decode", [buff], function(latexSource) {
              source.update_async(latexSource, new Date, "reload");
            });
            return true;
          }
        } else {
          ...
          if (http.responseText && http.responseText.length != 0) {
            source.update_async(http.responseText, new Date, "reload"); //.replace(/\r?\n/g,"\n")
            return true;
          }
        }
  ...
  if (/\.tex(?:$|#)/.test(url) && document.body) {
    ...
    if (pres.length > 0) {
      if (/^\s*<[!?]?[a-z_]+\b/i.test(pres[0].textContent)) {
        // this seems to be an HTML file
        if (!window.location.search || window.location.search == "") {
          var current = window.location.href;
          var target = current.replace(/#.*/, "") + "?" + window.location.hash;
