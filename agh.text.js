...
//*****************************************************************************
/**
 * Author: akinomygoa <myoga.murase@gmail.com>
 * Require: agh.js
 *
 * SYNOPSIS
 *
 *   @function String#format
 *     agh.Text.format が等価な機能を提供する。
 *   @function Number#format
 *   @function Boolean#format
 *   @function RegExp#format
 *   @function Date#format
 *   @class agh.StringReader(text)
 *     @function next()
 *     @member text
 *     @member index
 *     @member current
...
String.prototype.format = function() {
  ...
  return agh.Text.format.apply(null, args);
};
...
var num_sprintf = (function() {
  ...
  function trans_eform(ret) {
    if (ret.indexOf("e") < 0) switch (true) {
      ...
      case reg_fraction.test(ret):
        ret = ret.replace(reg_fraction, function($0, $H, $R, $F) {
          if (!$F) $F = "";
  ...
  function trans_fform(ret) {
    ...
    return ret.replace(reg_eform, function($0, $R, $F, $E) {
      if (!$F) $F = "";
      var e = parseInt($E);
      if (e == 0) return $R + "." + $F;
      if (e < 0) {
  ...
  function precision_f(str, prec, force_dot) {
    return str.replace(/([0-9]*)(?:\.([0-9]*))?/, function($0, $R, $F) {
      if (!$F) $F = "";
      if ($F.length < prec) {
  ...
  function precision_p(str, prec, force_dot) {
    return str.replace(/([0-9a-fA-F]*)(?:\.([0-9a-fA-F]*))?/, function($0, $R, $F) {
      if (!$F) $F = "";
      if ($F.length < prec) {
        $F += "0".repeat(prec - $F.length);
      } else if($F.length > prec) {
